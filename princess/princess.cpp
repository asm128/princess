...
// Returns 2 if deflected, 1 if hit and 
	...
	::pcs::attackCalculateDamage(lifeRemaining, damageDealt, damageReflected, attacker, attackedDefend);
	::pcs::SPointsLife																lifeDrained									= {};
	::pcs::attackApplyDrain(damageDealt, lifeDrained, attacker.DrainLife);

	attacker.Life.Health														+= lifeDrained.Health;
	attacker.Life.Shield														+= lifeDrained.Shield;
	attacked.Life.Health														= lifeRemaining.Health;
	attacked.Life.Shield														= lifeRemaining.Shield;
	return 0;
}
